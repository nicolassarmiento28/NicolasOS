...
- **Botón explícito de retorno**, además del control `□`: en la vista
  normal específicamente, sumar un botón con texto visible (ej. "Volver a
  terminal") en el cuerpo de la página, no solo el ícono `□` de la barra
  de título. El público de esta vista es justo el que no quiere lidiar
  con la metáfora de terminal — para esa persona, un ícono sin etiqueta es
  poca información. En la vista terminal el ícono `□` solo alcanza (ese
  usuario ya está cómodo con la interfaz de comandos), no hace falta
  duplicar ahí.

**Criterio de aceptación**: en la vista normal, además del control `□` en
la barra de título, existe un botón con texto legible que vuelve a la
vista terminal — visible sin necesidad de hacer scroll hasta el final de
la página (ej. cerca del ASCII banner, al principio).
