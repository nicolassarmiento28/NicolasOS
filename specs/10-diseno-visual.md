...
### Controles de la barra de título — estilo Windows, excepto linux — IMPLEMENTADO Y REVISADO
...
la derecha) en TODOS los temas EXCEPTO `linux`, que mantiene su estilo
actual (sin controles de ventana, barra simple sin decoración — ver
`03-temas.md`, es intencional que se vea austera y sin cromo).
...
- **`□` (maximizar/restaurar)** → reemplaza al botón separado "Vista
  ...
  forma de cambiar de vista, integrada al control de ventana en vez de un
  botón aparte.
...
tema `windows-xp` es la excepción: ahí sí pueden llevar el estilo bevelled
3D auténtico de la Luna theme de XP, ya que ese tema completo está
pensado para imitar Windows literalmente.

**Estado (revisión de diseno-visual, por lectura de código — sin Playwright
en esta ronda, pendiente de confirmación visual por qa-testing/onboarding-ux):**
implementado en `src/main.ts` (`.window-controls` con `.win-minimize`,
`.win-maximize`, `.win-close`) y `src/style.css` (`.window-controls` /
`.window-control`). Mecanismo aprobado:
- Alineación a la derecha vía `margin-left: auto` dentro del titlebar flex — OK.
- Ocultos en `linux` vía `--theme-window-controls: none` (token
  `showWindowControls: false` en `themes.ts`, ver `03-temas.md`), `flex`
  en el resto — cumple "excepto linux".
- Colores derivados de `--theme-titlebar-text` / `--theme-titlebar`, sin
  hex fijos de Windows real — cumple el requisito de paleta por tema.
- `□` reutiliza la lógica de cambio de vista existente; `_`/`X` solo
  imprimen easter egg sin acción real — coherente con el spec.
- El fix de contraste `titlebar-gradient` (cyberpunk) sigue vigente,
  ahora apuntando a `.window-control` en vez del botón viejo — válido,
  no se perdió cobertura al migrar de clase.

**Pendiente, no bloqueante** (el spec ya lo marca opcional arriba): el
bevelled 3D auténtico de windows-xp para los íconos no está implementado.
No se exige para aprobar esta tarea — queda como mejora futura si se
retoma windows-xp en `06-effects-v2.md`.

**Criterio de aceptación**: captura de Playwright confirma controles estilo
Windows (`_ □ X`) alineados a la derecha en cyberpunk, dos, windows-xp y
hacker; y confirma que `linux` NO los tiene (sigue con su barra simple).
Test de que `□` cambia de vista correctamente, y que `_`/`X` muestran su
mensaje de easter egg sin romper nada. (Verificación visual en navegador
real queda a cargo de onboarding-ux/qa-testing vía Playwright MCP, este
agente no lo tiene disponible.)
